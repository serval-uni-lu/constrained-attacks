...
do
  ...
  do
    ...
    do
      ...
      CUDA_VISIBLE_DEVICES=$DEVICES python run/scenarioC2.py --dataset_name $DATASET --model_name_source $MODEL_SRC --custom_path_source $MODEL_PATH_SRC --model_name_target $MODEL_TRG --custom_path_target $MODEL_PATH_TRG --attacks_name $ATTACK --max_eps $MAX_EPS --subset $SUBSET --batch_size $BATCH_SIZE --device $DEVICE 
