...
def run_experiment(model_source, model_target, dataset, scaler, x, y, args, device="cuda", save_examples: int = 1, xp_path="./data"):
    experiment = XP(args)
    ...
    attack = attack_class[0](constraints=constraints, scaler=scaler, model=model_source,
                             fix_equality_constraints_end=False, fix_equality_constraints_iter=False,
                             model_objective=model_source.predict_proba, **attack_args)
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        objective_calculator = ObjectiveCalculator(
            classifier=model_target.predict_proba,
...
def run(dataset_name: str, model_name_source: str, model_name_target: str, attacks_name: list[str] = None,
        max_eps: float = 0.1, subset: int = 1,
        batch_size: int = 1024, save_examples: int = 1, device: str = "cuda", custom_path_source: str = "",
        custom_path_target: str = ""):
    # Load data
    ...
    model_source = model_class.load_class(weight_path, x_metadata=metadata, scaler=scaler)
    ...
    weight_path = f"../models/constrained/{dataset_name}_{model_name_target}.model" if custom_path_target == "" else custom_path_target
    model_target = model_class.load_class(weight_path, x_metadata=metadata, scaler=scaler)
    ...
    for attack_name in attacks_name:
        ...
        run_experiment(model_source, model_target, dataset, scaler, x_test, y_test, args, device, save_examples)
...
if __name__ == "__main__":
    ...
    run(dataset_name=args.dataset_name, model_name1=args.model_name1, attacks_name=args.attacks_name.split("+"),
        subset=args.subset, custom_path_source=args.custom_path_source, model_name_target=args.model_name_target,
